...
def test_kth():
  ...
  for activity in activities:
    files[activity] = kth.listFiles(
        kth_dataset_path, activity=activity, abosultePath=True)
    print("activity: {} \t count: {}".format(activity, len(files[activity])))
  print(kth.metadata(kth_dataset_path))


def test_yolo():
  print("{}Testing yolo...{}".format(bg(1), attr('reset')))
  net = YoloV3(config["paths"]["yolov3"])
  print("Frames in the file {}".format(
      frameCount(kth.randomFile(kth_dataset_path))))
  i = 0
